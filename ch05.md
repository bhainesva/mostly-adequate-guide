...
```js
const dollarValues = map(prop('dollar_value'));
...
```js
const printIsFastest = flip(concat, ' is the fastest')
const sortByHP = sortBy(prop('horsepower'))
const fastestCar = compose(printIsFastest, prop('name'), last, sortByHP)
```
